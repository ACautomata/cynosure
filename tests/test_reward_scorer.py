...
class TestTrainingNoiseInjection:
    """训练期对称噪声注入（ADR-0009 决策 1-3，issue #104）。

    路径分流不变量：训练专用带噪前向（``training_patch_logits``）在
    归一化域加噪、仅参数更新路径消费；打分路径（``patch_logits`` /
    ``reward`` / held-out AUC）恒干净域——reward 每步 i.i.d. 抖动会经
    GRPO 组内标准化放大进 advantage（ADR-0009 决策 2 的否决理由）。
    σ_max = 0 是唯一关闭形态：与干净路径逐位一致且零 RNG 消耗（回归锚）。
    """

    def test_zero_sigma_matches_clean_path_bitwise(
        self, scenario: ScorerScenario,
    ) -> None:
        """回归锚：σ_max = 0 时带噪前向与干净打分前向逐位一致，且
        generator 零消耗（不采样——全链路与无注入代码行为一致）。"""
        scorer = scenario.scorer(disc_noise_sigma_max=0.0)
        latents = scenario.latents()
        generator = torch.Generator().manual_seed(42)
        state_before = generator.get_state().clone()
        noisy = scorer.training_patch_logits(latents, generator)
        assert torch.equal(noisy, scorer.patch_logits(latents))
        assert torch.equal(generator.get_state(), state_before)

    def test_positive_sigma_diverges_from_clean_path(
        self, scenario: ScorerScenario,
    ) -> None:
        """σ_max > 0 时带噪前向偏离干净路径（注入生效）且消耗 generator
        （噪声走注入的随机流，不碰进程全局 RNG）。"""
        scorer = scenario.scorer(disc_noise_sigma_max=0.2)
        latents = scenario.latents()
        generator = torch.Generator().manual_seed(42)
        state_before = generator.get_state().clone()
        noisy = scorer.training_patch_logits(latents, generator)
        assert not torch.equal(noisy, scorer.patch_logits(latents))
        assert not torch.equal(generator.get_state(), state_before)

    def test_scoring_path_stays_clean_under_injection(
        self, scenario: ScorerScenario,
    ) -> None:
        """打分路径分流不变量：注入开启（σ_max = 0.2）与关闭（0）的
        两个 scorer，同输入的 reward 与 patch_logits 逐位一致——噪声
        只存在于训练专用入口，reward / held-out AUC 留在干净域。"""
        latents = scenario.latents()
        clean = scenario.scorer(disc_noise_sigma_max=0.0)
        noisy = scenario.scorer(disc_noise_sigma_max=0.2)
        assert torch.equal(clean.reward(latents), noisy.reward(latents))
        assert torch.equal(
            clean.patch_logits(latents), noisy.patch_logits(latents),
        )

    def test_noise_replay_is_deterministic(self, scenario: ScorerScenario) -> None:
        """同 seed 同输入两次带噪前向逐位一致（续训重执行可复现的
        采样口径：噪声序列是 generator 状态的纯函数）。"""
        scorer = scenario.scorer(disc_noise_sigma_max=0.2)
        latents = scenario.latents()
        first = scorer.training_patch_logits(
            latents, torch.Generator().manual_seed(7),
        )
        second = scorer.training_patch_logits(
            latents, torch.Generator().manual_seed(7),
        )
        assert torch.equal(first, second)

    def test_per_sample_sigma_sampling_contract(
        self, scenario: ScorerScenario,
    ) -> None:
        """采样契约（ADR-0009 决策 3）：逐样本 σ ~ U[0, σ_max]、
        ε ~ N(0,1)，注入在归一化域（通道归一化之后）——同 generator
        seed 下按契约手工重放的噪声前向与实现逐位一致。σ 以相对通道
        std 的比例参数化：归一化域每通道 std = 1，噪声 std = σ × 通道
        std，免依赖 latent 存储域量级。"""
        sigma_max = 0.3
        scorer = scenario.scorer(disc_noise_sigma_max=sigma_max)
        latents = scenario.latents(batch=2)
        noisy = scorer.training_patch_logits(
            latents, torch.Generator().manual_seed(123),
        )
        # 手工重放：先逐样本 σ（rand，形状 [B]）、后 ε（randn，全形状）
        reference = torch.Generator().manual_seed(123)
        sigmas = torch.rand(2, generator=reference) * sigma_max
        buffers = dict(scorer.named_buffers())
        normalized = (latents - buffers["_normalizer.mean"]) / (
            buffers["_normalizer.std"]
        )
        eps = torch.randn(normalized.shape, generator=reference)
        expected = scorer.patch_logits(
            normalized + sigmas.view(-1, 1, 1, 1, 1) * eps,
        )
        assert torch.equal(noisy, expected)

    def test_noisy_forward_backprops_to_parameters(
        self, scenario: ScorerScenario,
    ) -> None:
        """带噪前向可反传：噪声是注入常数（非参数、阻断梯度），损失
        梯度照常流向判别器参数（参数更新路径的成立前提）。"""
        scorer = scenario.scorer(disc_noise_sigma_max=0.2)
        logits = scorer.training_patch_logits(
            scenario.latents(), torch.Generator().manual_seed(0),
        )
        logits.sum().backward()
        weight = next(
            p for p in scorer.discriminator.parameters() if p.ndim == 5
        )
