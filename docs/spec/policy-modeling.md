...
| 项 | 值 | 备注 |
|---|---|---|
| 组大小 G | 12 | 固定——G 是算法口径（组内 advantage 的样本数）而非显存旋钮，不为显存下调；显存压力经**顺序分块**消化：rollout 续跑的 G 方向整批前向按激活预算切子批**顺序**积分（`policy.forward_activation_budget_gib`，缺省按设备总显存自动探测），峰值以子批为界（#123 首跑 OOM 修复）。降 G 仅作改变算法口径的最后手段 |
| advantage clamp | ±5 | |
