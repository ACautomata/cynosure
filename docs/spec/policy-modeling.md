...
| 项 | 值 | 备注 |
|---|---|---|
| 组大小 G | 12 | 固定——G 是算法口径（组内 advantage 的样本数）而非显存旋钮，不为显存下调；显存压力经**顺序分块**消化：rollout 续跑的 G 方向整批前向按激活预算切子批**顺序**积分（`policy.forward_activation_budget_gib`，缺省按设备总显存自动探测），峰值以子批为界（#123 首跑 OOM 修复）。分布式下分块上限经 `all_reduce_min` 取**全 rank 最小**后再截断（#165 review P1：条件逐 rank 独立采样使本地上限异值，FSDP 集合序列绑定前向调用次数——本地各自取值即互等挂死）。降 G 仅作改变算法口径的最后手段 |
| advantage clamp | ±5 | |
