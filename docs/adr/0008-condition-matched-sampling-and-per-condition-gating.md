...
## 修订（2026-09-17，维护者裁决）：条件闸总开关

**背景**：#122 在 MR-RATE 小规模真实数据上产出**空白名单**的报告（11 条件
held-out AUC 全落 chance 带 0.4934–0.5203，门槛 0.65 不可达），而决策 5 的
「空白名单拒绝开跑」是硬前置——RL 主循环 tracer（#123）因此在首跑前即被拒，
无法产出任何主循环执行序与成本读数；而「AUC 是否适合作门控信号」本身正是
需要用完整训练读数裁决的问题。

**裁决**：决策 5/7/8 的 AUC 门控链（上岗判定 + 逐 iteration 白名单门控 +
EMA 动态恢复）合并为一个可关闭的总开关 `reward.condition_gate_enabled`
（默认 `true` = 本 ADR 原口径，行为零变化）。关闭时：

- 上岗判定不再拒绝开跑（`ReadinessGate.check` 直接放行），运行时白名单
  退化为「不设条件闸」的全条件放行占位；判别器 warm-start 的权重装载
  （ADR-0007）**不受影响**——它不属门控链；
- 逐 iteration 门控停止（`DynamicWhitelist.observe` 恒返回「无 iteration
  被跳过」），动态恢复评估停步（名单无进出、无「恢复」可言）；集体回合
  照走，全 rank 执行序不变；
- **观测面不退化**：held-out AUC 照常逐 iteration 测量并落 `iter` 事件的
  `heldout_auc` 字段，分叉监控（ADR-0009）照常。关闸关的是「AUC 驱动
  更新决定」，不是「AUC 被测量」——完整训练的 AUC 读数正是本开关的
  裁决输入。

**边界**：本开关是**诊断/执行期**口径，不改变决策 1–4、6 的任何语义
（条件匹配采样、容量守卫、支撑度规则原样）；ADR-0008 仍是既定协议，关闸
