...
测试形态：fixture 多进程（CPU gloo）经 ``torch.multiprocessing.spawn`` 起
world 个 worker，每个 worker 以 torchrun 同款环境变量（RANK/LOCAL_RANK/
WORLD_SIZE/MASTER_ADDR/MASTER_PORT）驱动真实 CLI 路径——torchrun 的
worker 入口即 ``python -m cynosure.cli``，本 harness 与其进程语义等价
（--nproc_per_node 的参数化 = world size；torchrun 二进制的 rendezvous
冒烟属集群 M0 门槛清单，本机 CPU fixture 不依赖它）。
...
_NEXT_PORT = [29730]
"""spawn harness 的 TCPStore 端口游标（串行递增，避 TIME_WAIT 冲突）。"""
...
def _dist_train_worker(
    rank: int, world: int, port: int, argv: list[str], queue,
) -> None:
    """torchrun worker 的进程内等价入口：env 注入后驱动真实 CLI。

    结果（退出码 + stderr）经队列回传主进程（worker 崩溃也回传，
    死锁由主进程 join 超时兜底）。
    """
    os.environ.update(
        RANK=str(rank),
...
def spawn_train(
    config_path: Path, run_dir: Path, world: int, argv: list[str] | None = None,
) -> DistTrainResult:
    """world 个 worker 并行跑 ``train``（torchrun 语义入口）。"""
    _NEXT_PORT[0] += 1
    if argv is None:
        argv = [
    ...
    processes = [
        context.Process(
            target=_dist_train_worker,
            args=(rank, world, _NEXT_PORT[0], argv, queue),
        )
    ...
    return DistTrainResult(
        codes=[collected[rank]["code"] for rank in range(world)],
        errors=[collected[rank]["stderr"] for rank in range(world)],
    )
...
def _assert_trajectories_equivalent(
    left: list[dict], right: list[dict],
) -> None:
    """跨路径（分布式 vs 单进程）的轨迹等价：结构字段严格一致、浮点
    字段在重算路径噪声容差内一致（同路径重放才逐位，见 _EQUIVALENCE_RTOL）；
    wall-clock elapsed_s 不参与对比。"""
    ...
    for first, second in zip(left, right):
        ...
        for key in first:
            ...
            if isinstance(a, float) and isinstance(b, float):
                assert math.isclose(
                    a, b, rel_tol=_EQUIVALENCE_RTOL, abs_tol=1e-8,
                ), f"{key}: {a} vs {b}"
                ...
                for name in a:
                    assert math.isclose(
                        a[name], b[name],
                        rel_tol=_EQUIVALENCE_RTOL, abs_tol=1e-8,
                    ), f"{key}:{name}: {a[name]} vs {b[name]}"
            else:
                assert a == b, f"{key}: {a!r} vs {b!r}"


def _assert_checkpoints_equivalent(left: Path, right: Path, name: str) -> None:
    ...
    for key in a:
        assert torch.allclose(
            a[key], b[key], rtol=_EQUIVALENCE_RTOL, atol=1e-7,
        ), f"{name}:{key} 超出等价容差（max diff "
        f"{(a[key] - b[key]).abs().max().item():.3e}）"


def _load_rank_state(run_dir: Path, rank: int) -> dict:
    return torch.load(
        run_dir / "checkpoints" / f"resume_state_rank{rank}.pt",
        map_location="cpu", weights_only=True,
    )


def _assert_states_bitwise_identical(
    run_dir: Path, world: int, keys: list[str],
) -> None:
    """各 rank 续训状态的指定字段逐位一致（同步生效的外部观测面）。"""
    states = [_load_rank_state(run_dir, rank) for rank in range(world)]
    for key in keys:
...
class TestSingleRankEquivalence:
    ...
    def test_world1_entry_matches_in_process_run(
        ...
        result = spawn_train(scenario.config_path, scenario.run_dir, world=1)
        result.assert_green()
        ...
        _assert_trajectories_equivalent(dist_events, inproc_events)

        for name in ("policy_iter2.pt", "discriminator_iter2.pt"):
            _assert_checkpoints_equivalent(
                scenario.run_dir, reference_dir, name,
            )
...
class TestTwoRankSharding:
    ...
    def test_two_rank_run_merges_metrics_and_syncs_weights(
        ...
        result = spawn_train(scenario.config_path, scenario.run_dir, world=2)
        result.assert_green()
        ...
        _assert_states_bitwise_identical(
            scenario.run_dir, world=2,
        ...
        synced = _load_rank_state(scenario.run_dir, 0)["discriminator_network"]
        assert any(
...
class TestTwoRankResume:
    ...
    def _resume_run(self, scenario: TrainingLoopScenario) -> DistTrainResult:
        argv = [
            "train", "--config", str(scenario.config_path),
            "--run-dir", str(scenario.run_dir), "--resume",
        ]
        return spawn_train(scenario.config_path, scenario.run_dir, world=2, argv=argv)
    ...
    def test_resume_to_identical_trajectory(
        ...
        spawn_train(scenario.config_path, scenario.run_dir, world=2).assert_green()
        assert [
            event["iteration"]
        ...
        self._resume_run(scenario).assert_green()
        ...
        _patch_config(scenario, schedule={"max_iterations": 4})
        spawn_train(scenario.config_path, baseline_dir, world=2).assert_green()
        ...
        for rank in range(2):
            resumed = _load_rank_state(scenario.run_dir, rank)
            baseline = _load_rank_state(baseline_dir, rank)
            for key in ("policy_network", "discriminator_network"):
                for name in resumed[key]:
                    assert torch.equal(
                        resumed[key][name], baseline[key][name],
                    ), f"rank {rank} {key}:{name} 续训后漂移"
            assert resumed["iteration"] == baseline["iteration"] == 4
