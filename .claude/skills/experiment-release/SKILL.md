...
description: 实验完成后把 run 目录结果与运行源码打包发布为 GitHub Release（tag exp/<run-dir名>，源码 tarball 按运行 commit 用 git archive 固定）。触发：一次 train/eval run 跑完需要发布结果；用户要求发布/上传实验结果或建实验 release。
---

# 实验发布：run 目录 → GitHub Release

一次发布 = 一个 run 目录 + 产生它的源码 commit，绑成一个 GitHub Release。核心是
...
run 目录须在本地（还在 SothisAI 实例上的先按 sugon 惯例拉回）。三件套齐才算可发布：
...
- 结果：run 目录下的 `config.json`、`metrics.jsonl`、`manifest.json`，以及
  除 `checkpoints/` 外的其他结果文件（图表、评测报告等）；
...
是否 early-stop；标出源码 commit short sha。写入临时文件供 `--notes-file`。
